...
public class WitchAltarBlockEntity extends BlockEntity implements Inventory {
    ...
    public WitchAltarBlockEntity(BlockPos pos, BlockState state) {
        super(WitcheryBlockEntityTypes.WITCH_ALTAR, pos, state);
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, WitchAltarBlockEntity blockEntity) {
        if (world != null && !world.isClient) {
            if (blockEntity.loadingTimer > 0) {
                ...
                blockEntity.scan(40);
                if (world.getTime() % 20 == 0) {
                    if (world.getBlockState(pos.up()).getBlock() == WitcheryBlocks.BLESSED_STONE) {
                    ...
                    PlayerLookup.around((ServerWorld) world, Vec3d.of(pos), 24).forEach(player -> {
                        if (!WitcheryComponents.CURSES_COMPONENT.get(player).hasCurse(WitcheryCurses.APATHY) && WitcheryAPI.fillMagic(player, 5, true) && blockEntity.drain(10, true)) {
                            WitcheryAPI.fillMagic(player, 5, false);
    ...
    private static boolean givesAltarPower(BlockState state) {
        ...
        return state.isIn(WitcheryTags.GIVES_ALTAR_POWER);
    }

    private static float getSwordValue(ItemStack stack) {
        return stack.isIn(WitcheryTags.WEAK_SWORDS) ? 1.1F : stack.isIn(WitcheryTags.AVERAGE_SWORDS) ? 1.2F : stack.isIn(WitcheryTags.STRONG_SWORDS) ? 1.3F : 1;
    }

    private static int getPentacleValue(ItemStack stack) {
        return stack.isIn(WitcheryTags.WEAK_PENTACLES) ? 2 : stack.isIn(WitcheryTags.AVERAGE_PENTACLES) ? 3 : stack.isIn(WitcheryTags.STRONG_PENTACLES) ? 5 : 1;
    }

    private static int getWandValue(ItemStack stack) {
        return stack.isIn(WitcheryTags.WEAK_WANDS) ? 40 : stack.isIn(WitcheryTags.AVERAGE_WANDS) ? 80 : stack.isIn(WitcheryTags.STRONG_WANDS) ? 120 : 0;
    }
}
