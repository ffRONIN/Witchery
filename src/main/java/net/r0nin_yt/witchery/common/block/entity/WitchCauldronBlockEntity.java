...
import net.r0nin_yt.witchery.common.recipe.CauldronBrewingRecipe;
import net.r0nin_yt.witchery.common.recipe.OilRecipe;
...
//	private static final MutableText DEFAULT_NAME = Text.translatable(WitcheryBlocks.WITCH_CAULDRON.getTranslationKey());
	...
	public WitchCauldronBlockEntity(BlockPos pos, BlockState state) {
		super(WitcheryBlockEntityTypes.WITCH_CAULDRON, pos, state);
	}
	...
	public static void tick(World world, BlockPos pos, BlockState state, WitchCauldronBlockEntity blockEntity) {
		if (world != null) {
			if (!blockEntity.loaded) {
				...
				blockEntity.oilRecipe = world.getRecipeManager().listAllOfType(WitcheryRecipeTypes.OIL_RECIPE_TYPE).stream().filter(recipe -> recipe.matches(blockEntity, world)).findFirst().orElse(null);
				blockEntity.loaded = true;
			}
			blockEntity.heatTimer = MathHelper.clamp(blockEntity.heatTimer + (state.get(Properties.LIT) && state.get(WitcheryProperties.LEVEL) > 0 ? 1 : -1), 0, 160);
			if (!world.isClient) {
				if (blockEntity.heatTimer >= 60 && state.get(WitcheryProperties.LEVEL) > 0) {
					...
					if (world.getTime() % 5 == 0) {
						world.getEntitiesByType(EntityType.ITEM, blockEntity.box, entity -> true).forEach(itemEntity -> {
							if (itemEntity.getStack().getItem() == WitcheryItems.WOOD_ASH || state.get(WitcheryProperties.LEVEL) == 3) {
	...
	public Mode reset() {
		if (world != null) {
			...
			clear();
			oilRecipe = null;
			world.setBlockState(pos, getCachedState().with(WitcheryProperties.LEVEL, 0));
		}
		return Mode.NORMAL;
	}
	...
	private Mode insertStack(ItemStack stack) {
		if (world != null) {
			if (stack.getItem() == WitcheryItems.WOOD_ASH) {
				Mode reset = reset();
				syncCauldron();
				return reset;
				...
				if (firstEmpty != -1) {
					...
					if (mode == Mode.NORMAL && stack.getItem() == WitcheryItems.MANDRAKE_ROOT) {
						clear();
						return Mode.BREWING;
					}
					if (mode == Mode.BREWING) {
						CauldronBrewingRecipe cauldronBrewingRecipe = world.getRecipeManager().listAllOfType(WitcheryRecipeTypes.CAULDRON_BREWING_RECIPE_TYPE).stream().filter(recipe -> recipe.input.test(stack)).findFirst().orElse(null);
						...
						oilRecipe = world.getRecipeManager().listAllOfType(WitcheryRecipeTypes.OIL_RECIPE_TYPE).stream().filter(recipe -> recipe.matches(this, world)).findFirst().orElse(null);
						if (oilRecipe != null) {
	...
	public ItemStack getPotion(LivingEntity creator) {
		...
		if (world != null) {
			...
			int durationBoost = creator != null && WitcheryUtil.getArmorPieces(creator, armorStack -> armorStack.getItem() instanceof ArmorItem armorItem && armorItem.getMaterial() == WitcheryMaterials.ALCHEMIST_ARMOR) >= 3 ? 1 : 0;
			int amplifierBoost = 0;
			boolean leonard = creator instanceof PlayerEntity player && WitcheryAPI.isPledged(player, WitcheryPledges.LEONARD);
			for (int i = 0; i < size(); i++) {
		...
		//		}
				CauldronBrewingRecipe cauldronBrewingRecipe = world.getRecipeManager().listAllOfType(WitcheryRecipeTypes.CAULDRON_BREWING_RECIPE_TYPE).stream().filter(recipe -> recipe.input.test(stackInSlot)).findFirst().orElse(null);
			...
			for (int i = effects.size() - 1; i >= 0; i--) {
				if (effects.get(i).getEffectType() == WitcheryStatusEffects.CORRUPTION) {
					finalEffects.add(effects.remove(i));
				} else if (effects.get(i).getEffectType() == WitcheryStatusEffects.POLYMORPH) {
	...
	public int getTargetLevel(ItemStack stack) {
		...
		int level = getCachedState().get(WitcheryProperties.LEVEL);
		if (mode == Mode.NORMAL) {
