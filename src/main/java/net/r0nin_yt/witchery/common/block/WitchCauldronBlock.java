...
public class WitchCauldronBlock extends Block implements BlockEntityProvider, Waterloggable {
	...
	public WitchCauldronBlock(Settings settings) {
		super(settings);
		setDefaultState(getDefaultState().with(Properties.WATERLOGGED, false).with(WitcheryProperties.LEVEL, 0));
	}
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
		return super.getPlacementState(ctx).with(Properties.WATERLOGGED, ctx.getWorld().getFluidState(ctx.getBlockPos()).getFluid() == Fluids.WATER).with(Properties.LIT, ctx.getWorld().getBlockState(ctx.getBlockPos().down()).isIn(WitcheryTags.HEATS_CAULDRON));
	}
	...
	public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
		if (state.get(Properties.WATERLOGGED)) {
			...
			state = state.with(WitcheryProperties.LEVEL, 0);
			state = state.with(Properties.LIT, false);
		} else {
			state = state.with(Properties.LIT, world.getBlockState(pos.down()).isIn(WitcheryTags.HEATS_CAULDRON));
		}
			...
			if (nameTag || bucket || waterBucket || glassBottle || waterBottle) {
				if (!client) {
					if (nameTag && stack.hasCustomName()) {
						...
						if (targetLevel > -1) {
							if (bucket) {
								WitcheryUtil.addItemToInventoryAndConsume(player, hand, new ItemStack(Items.WATER_BUCKET));
							} else if (waterBucket) {
								WitcheryUtil.addItemToInventoryAndConsume(player, hand, new ItemStack(Items.BUCKET));
							} else if (glassBottle) {
								...
								if (bottle != null) {
									WitcheryUtil.addItemToInventoryAndConsume(player, hand, bottle);
								}
							} else if (waterBottle) {
								WitcheryUtil.addItemToInventoryAndConsume(player, hand, new ItemStack(Items.GLASS_BOTTLE));
							}
							if (targetLevel == 0) {
								cauldron.mode = cauldron.reset();
							}
							world.setBlockState(pos, state.with(WitcheryProperties.LEVEL, targetLevel));
	...
	public int getComparatorOutput(BlockState state, World world, BlockPos pos) {
		return state.get(WitcheryProperties.LEVEL);
	}
	...
	public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
		if (world.getBlockState(pos).get(WitcheryProperties.LEVEL) > 0 && entity instanceof LivingEntity) {
	...
	protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
		builder.add(Properties.WATERLOGGED, WitcheryProperties.LEVEL, Properties.LIT);
	}
}
