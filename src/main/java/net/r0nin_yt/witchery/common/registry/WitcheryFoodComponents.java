...
public class WitcheryFoodComponents {
	public static final FoodComponent DEMON_HEART = new FoodComponent.Builder().hunger(8).saturationModifier(0.6f).meat().alwaysEdible().statusEffect(new StatusEffectInstance(WitcheryStatusEffects.CORRUPTION, 1), 1).statusEffect(new StatusEffectInstance(StatusEffects.WITHER, 800, 2), 1).statusEffect(new StatusEffectInstance(StatusEffects.NAUSEA, 800, 2), 1).statusEffect(new StatusEffectInstance(StatusEffects.HUNGER, 800, 2), 1).build();
