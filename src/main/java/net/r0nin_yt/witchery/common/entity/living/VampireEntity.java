...
public class VampireEntity extends WitcheryHostileEntity {
	...
	public static boolean canSpawn(EntityType<VampireEntity> type, ServerWorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
		return world.toServerWorld().isNight() && WitcheryAPI.getMoonPhase(world.toServerWorld().toServerWorld()) == 4 && HostileEntity.canSpawnInDark(type, world, spawnReason, pos, random);
	}
	...
	public void tick() {
		super.tick();
		if (!world.isClient) {
            WitcheryComponents.BLOOD_COMPONENT.maybeGet(this).ifPresent(bloodComponent -> {
	...
	protected SoundEvent getAmbientSound() {
		return WitcherySoundEvents.ENTITY_VAMPIRE_AMBIENT;
	}

	@Override
	protected SoundEvent getHurtSound(DamageSource source) {
		return WitcherySoundEvents.ENTITY_VAMPIRE_HURT;
	}

	@Override
	protected SoundEvent getDeathSound() {
		return WitcherySoundEvents.ENTITY_VAMPIRE_DEATH;
	}
