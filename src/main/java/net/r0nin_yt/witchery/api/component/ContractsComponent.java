...
public class ContractsComponent implements ServerTickingComponent {
	...
	public void readFromNbt(NbtCompound tag) {
		...
		for (int i = 0; i < contractsList.size(); i++) {
			...
			addContract(new Contract.Instance(WitcheryRegistries.CONTRACTS.get(new Identifier(contractCompound.getString("Contract"))), contractCompound.getInt("Duration"), contractCompound.getInt("Cost")));
		}
	}
	...
	public void serverTick() {
		...
		if (level > 0) {
			obj.addStatusEffect(new StatusEffectInstance(WitcheryStatusEffects.PACT, 10, level - 1, true, false));
	...
	public NbtList toNbtContract() {
		...
		for (Contract.Instance instance : getContracts()) {
			...
			contractCompound.putString("Contract", WitcheryRegistries.CONTRACTS.getId(instance.contract).toString());
