...
package net.r0nin_yt.witchery.api.block;
...
public class WitchAltarBlock extends HorizontalFacingBlock implements BlockEntityProvider, Waterloggable {
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		if (!formed) {
			AltarMapEntry entry = WitcheryAPI.ALTAR_MAP_ENTRIES.stream().filter(e -> e.unformed == this && e.carpet == stack.getItem()).findFirst().orElse(null);
			if (entry != null) {
				if (!client) {
			...
			if (!client) {
				if (!stack.isEmpty()) {
					boolean sword = stack.isIn(WitcheryTags.SWORDS);
					boolean pentacle = stack.isIn(WitcheryTags.PENTACLES);
					boolean wand = stack.isIn(WitcheryTags.WANDS);
					...
					if (player.isSneaking()) {
						...
						player.sendMessage(Text.of(blockEntity.power + " / " + blockEntity.maxPower + " (" + blockEntity.gain + "x)"), true);
					}
				}
			}
			return ActionResult.success(client);
	...
	public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
		if (formed) {
			...
			for (BlockPos foundPos : WitcheryUtil.getBlockPoses(pos, WitcheryConfig.altarDistributionRadius, currentPos -> world.getBlockEntity(currentPos) instanceof UsesAltarPower)) {
	...
	public static BlockPos getClosestAltarPos(World world, BlockPos pos) {
		return WitcheryUtil.getClosestBlockPos(pos, WitcheryConfig.altarDistributionRadius, currentPos -> world.getBlockEntity(currentPos) instanceof WitchAltarBlockEntity);
	}
