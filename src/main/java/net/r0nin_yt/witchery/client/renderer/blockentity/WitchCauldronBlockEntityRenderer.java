...
package net.r0nin_yt.witchery.client.renderer.blockentity;
...
import net.r0nin_yt.witchery.common.registry.WitcheryParticleTypes;
...
public class WitchCauldronBlockEntityRenderer implements BlockEntityRenderer<WitchCauldronBlockEntity> {
	...
	public void render(WitchCauldronBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		...
		if (world != null) {
			...
			int level = entity.getCachedState().get(WitcheryProperties.LEVEL);
			if (level > 0) {
				matrices.push();
	...
	//			renderWater(entity.color, matrices, vertexConsumers, light, overlay, SpriteIdentifiers.CAULDRON_WATER.getSprite());
				...
				if (entity.heatTimer >= 60 && !MinecraftClient.getInstance().isPaused()) {
					float fluidHeight = 0;
					float width = 0.35f;
					switch (entity.getCachedState().get(WitcheryProperties.LEVEL)) {
					...
					if (fluidHeight > 0) {
						...
						world.addParticle((ParticleEffect) WitcheryParticleTypes.CAULDRON_BUBBLE, pos.getX() + 0.5 + MathHelper.nextDouble(world.random, -width, width), pos.getY() + fluidHeight, pos.getZ() + 0.5 + MathHelper.nextDouble(world.random, -width, width), ((entity.color >> 16) & 0xff) / 255f, ((entity.color >> 8) & 0xff) / 255f, (entity.color & 0xff) / 255f);
					}
				}
			}
		}
